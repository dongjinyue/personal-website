...
- `docs/database-design.md` 已覆盖实体职责、6 张表数据字典、关系、删除策略和命名约定；
- `database/day-09-practice.sql` 已包含约束、索引、种子数据、关联关系和安全 CRUD；
- 整份 SQL 已在临时 SQLite 内存数据库中成功执行；
- 执行结果为 4 个工具、5 个标签、7 条工具标签关联；
- 临时删除练习没有影响真实工具数据；
- 数据结构可以继续用于 Day 10 的 Prisma Schema（Prisma 模式）设计。

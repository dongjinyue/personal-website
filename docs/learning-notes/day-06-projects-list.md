...
- [x] 已定义 `ProjectStatus` 联合类型；
- [x] Project 数据包含 `status` 和 `tags`；
- [x] 每条项目数据都符合扩展后的模型；
- [x] ProjectCard 支持状态、标签和外部链接；
- [x] 状态通过集中映射显示中文；
- [x] 标签使用 `ul/li` 和 `map`；
- [x] 标签使用稳定 key；
- [x] coverImage 缺少时显示 Fallback；
- [x] 没有引用不存在的图片文件；
- [x] GitHub 链接只在地址存在时显示；
- [x] 打开项目链接只在地址存在时显示；
- [x] 没有添加尚不能使用的详情链接；
- [x] 首页保持简洁卡片；
- [x] Projects 页面展示完整卡片；
...
- 静态 UI 审查：通过，0 个阻塞问题；
- 首页桌面端：4 张内容卡片，无状态和标签，保持简洁；
- Projects 桌面端：3 张项目卡片、3 组标签，状态为“已完成、开发中、开发中”；
- Projects 手机端：390px 宽度无横向溢出；
- 外部链接：只显示数据中真实存在的 1 个 GitHub 链接；
- 详情链接：0 个，符合 Day 7 前不提供无效入口的要求；
- 因当前终端找不到 Node.js/npm，`lint`、TypeScript 类型检查和生产构建仍需学习者在自己的终端执行。

# 个人网站每日学习复习笔记

这个目录用于保存个人网站项目每天的学习总结。每完成一天，就新增一份对应的复习文档，方便以后按天回顾，而不需要重新翻找聊天记录。

## 使用方法

建议每次学习按下面的顺序进行：

```text
开始当天学习
↓
理解概念并动手编写代码
↓
完成当天验收
↓
补充当天复习文档
↓
第二天开始前快速复习
```

复习时不要只看答案。可以先尝试回答文档里的“自测题”，再查看前面的知识说明。

## 学习目录

| 天数 | 主题 | 状态 | 复习文档 |
| --- | --- | --- | --- |
| Day 1 | Next.js 基础与 App Router（应用路由） | 已完成 | [查看 Day 1](./day-01-nextjs-and-app-router.md) |
| Day 2 | TypeScript（类型化 JavaScript）、Component（组件）与 Props（组件参数） | 已完成 | [查看 Day 2](./day-02-components-and-props.md) |
| Day 3 | 首页 | 已完成 | [查看 Day 3](./day-03-homepage.md) |
| Day 4 | CSS、Flexbox、Grid 与 Responsive Design（响应式设计） | 已完成 | [查看 Day 4](./day-04-css-responsive-design.md) |
| Day 5 | TypeScript 数据模型与数据驱动页面 | 已完成 | [查看 Day 5](./day-05-data-models.md) |
| Day 6 | Projects 列表与项目卡片设计 | 已完成 | [查看 Day 6](./day-06-projects-list.md) |
| Day 7 | Project Detail（项目详情）与 Dynamic Route（动态路由） | 进行中 | [查看 Day 7](./day-07-project-detail-dynamic-route.md) |
| Day 8 | Tools（工具集） | 未开始 | 完成学习后创建 |
| Day 9 | Database（数据库）基础 | 未开始 | 完成学习后创建 |
| Day 10 | Prisma（数据库 ORM 工具）与 Database（数据库） | 未开始 | 完成学习后创建 |
| Day 11 | 管理员登录 | 未开始 | 完成学习后创建 |
| Day 12 | Dashboard（管理后台） | 未开始 | 完成学习后创建 |
| Day 13 | Tools CRUD（工具增删改查） | 未开始 | 完成学习后创建 |
| Day 14 | Projects CRUD（项目增删改查）与公开/私有 | 未开始 | 完成学习后创建 |
| Day 15 | Build（构建）与 Deployment（部署） | 未开始 | 完成学习后创建 |
| Day 16 | Domain（域名）、DNS（域名解析）与最终验收 | 未开始 | 完成学习后创建 |

## 每日文档固定结构

以后每天的复习文档尽量包含：

1. 今天完成了什么；
2. 必须理解的核心概念；
3. 代码与目录之间的关系；
4. 常见错误；
5. 自测题；
6. 当天验收清单；
7. 一句话总结。

...
项目管理延续同一表单、表格、分页和确认弹窗语言。公开状态使用文字徽标，并与项目进度、首页推荐明确分开；新建项目默认私有，公开或设回私有只能在编辑页的独立区域确认。基础表单有未保存修改时禁用可见性切换，避免刷新丢稿；已公开项目显示即时生效提醒。公开网站始终使用无用户会话的数据客户端，管理员浏览公开页面时也保持访客视角。

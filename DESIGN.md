...
按钮需要默认、悬停、按下、键盘焦点、禁用和等待状态。认证字段使用真实标签、浏览器自动填充语义和可见焦点；错误在表单内以文字展示，并通过状态区域通知辅助技术。后台导航使用真实链接并通过 `aria-current` 表达当前位置，刷新使用原生按钮；只读预览使用语义化表格，明确当前行数、总数、排序和时区。所有后台查询先独立验证管理员，不以布局或导航可见性代替权限检查。

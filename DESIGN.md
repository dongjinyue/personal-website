...
视觉记忆点是首页内容索引带；它只呈现可浏览的信息，不使用圆形轨道、雷达、漂浮点或游戏 HUD 式装饰。认证流程不复制装饰，避免让安全操作看起来像营销落地页。避免霓虹暗色后台、重阴影卡片和无意义渐变。运行时颜色以 `app/globals.css` 的 CSS 变量为唯一实现来源，本文件记录这些现有值和使用理由，不生成第二套令牌。
...
顶部“工具集”采用宽幅分类索引菜单，以真实公开分类作为结构，不用装饰图形占位；项目菜单继续保持紧凑单列。图标沿用细描边 SVG，并在有歧义时保留文字标签。动效只反馈悬停和按下，`prefers-reduced-motion`（减少动态效果偏好）下取消过渡。界面文案使用直接的动作词和可恢复的错误提示，不暴露密码、Cookie 或令牌。

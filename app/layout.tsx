...
export default async function RootLayout({
  ...
  let navigationKnowledgeGroups: KnowledgeNavigationGroup[] = [];
  // 身份与导航内容并行读取，避免每次页面跳转串行等待多个网络请求。
  const [userResult, projectsResult, toolsResult, categoriesResult, knowledgeResult] = await Promise.allSettled([
    getCurrentUserIdFromClaims(), getNavigationPublicProjects(),
    getNavigationPublicTools(), getPublicToolCategories(),
    getNavigationKnowledgeGroupsForCurrentUser(),
  ]);
  ...
  if (knowledgeResult.status === "fulfilled") navigationKnowledgeGroups = knowledgeResult.value;
      ...
      <body>
        <UnsavedChangesProvider>
          <Header showAdmin={showAdmin} projects={navigationProjects} tools={navigationTools}
            categories={navigationCategories} knowledgeGroups={navigationKnowledgeGroups} />
          {children}
          <Footer />
        </UnsavedChangesProvider>
      </body>

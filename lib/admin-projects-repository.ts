...
export async function getAdminProjectsPage(rawPage?: string) {
  ...
  const { data, error } = await supabase.from("projects")
    .select("id, slug, name, status, hide_from_guests, is_featured, updated_at")
...
export async function getAdminProject(id: string) {
  ...
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.from("projects")
    .select("id, slug, name, description, long_description, status, project_url, github_url, is_featured, hide_from_guests, created_at, updated_at")

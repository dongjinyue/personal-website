...
export async function getAdminToolsPage(rawPage?: string) {
  ...
  const { data, error } = await supabase
    .from("tools")
    .select("id, name, description, url, category, is_favorite, is_public, hide_from_guests, created_at, updated_at, tool_tags(tags(name))")
...
export async function getAdminTool(id: string) {
  ...
  const { data, error } = await supabase
    .from("tools")
    .select("id, name, description, url, category, is_favorite, is_public, hide_from_guests, created_at, updated_at")
    .eq("id", id)
    .maybeSingle();
